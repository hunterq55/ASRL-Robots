...
class MotorMatlab : public LibraryBase //add-on class
{
    ...
    void loop()
    {
        if(pMotor[0] != NULL)
        {
            inputRadSec1 = pMotor[0]->getRadSec();
            pMotor[0]->Compute();
            pMotor[0]->setDuty(1000);
            //debugPrint(PID_BUGS,outputVoltage1);
        }
        
        if(pMotor[1] != NULL)
        {
            inputRadSec2 = pMotor[1]->getRadSec();
            pMotor[1]->Compute();
            pMotor[1]->setDuty(outputVoltage2);
        }
        
        if(pMotor[2] != NULL)
        {
            inputRadSec3 = pMotor[2]->getRadSec();
            pMotor[2]->Compute();
            pMotor[2]->setDuty(outputVoltage2);
        }
    }
    ...
    void commandHandler(byte cmdID,byte* dataIn,unsigned int payload_size)
    {
        switch (cmdID)
        {
            case MOTOR_CREATE:
            {
                ...
                if(ID[0] == 0)
                {
                    pMotor[ID[0]] = new Motor(pinNumbers[0],pinNumbers[1],
